...
const navigate = (start, end, minute) => {
  let openPositions = [{ x: start.x, y: start.y, minute }];
...
const there = navigate({ x: 0, y: 0 }, { x: cols - 1, y: rows - 1 }, 0);
const back = navigate({ x: cols - 1, y: rows - 1 }, { x: 0, y: 0 }, there);
const again = navigate({ x: 0, y: 0 }, { x: cols - 1, y: rows - 1 }, back);
