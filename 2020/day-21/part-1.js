...
function main(input) {
  ...
  const ingredients = foods.flatMap((f) => f.ingredients).sort();
  ...
  const nonAllergenIngredients = difference(
    ingredients,
    uniqueAllergens.flatMap((ua) => ua.ingredients),
  );

  console.log({ count: nonAllergenIngredients.length });
}
